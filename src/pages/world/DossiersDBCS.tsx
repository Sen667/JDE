...
import { Search, FileText, Plus, Filter, Download, MoreHorizontal, Eye, Calendar, Mail, BarChart3, Database, Trash2 } from 'lucide-react';
...
const DossiersDBCS = () => {
  ...
  const fetchWorldAndDossiers = async () => {
    try {
      // Fetch DBCS world data using Laravel API
      const worldsResponse = await dossierAPI.getDossiers({ world: 'DBCS' });
      if (worldsResponse.dossiers) {
        ...
        // Set dossiers filtered by DBCS world
        setDossiers(worldsResponse.dossiers.map((d: any) => ({
          ...d,
          owner: d.owner || { display_name: 'Inconnu' },
        })));
      } else {
        // Fallback to hardcoded DBCS world data if no dossiers exist
        setWorld({
          id: '3', // Default ID for DBCS
          code: 'DBCS',
          name: 'Base de Connaissance et Statistiques',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-100 via-green-50/50 to-background dark:from-green-950/40 dark:via-green-900/20 dark:to-background p-6">
